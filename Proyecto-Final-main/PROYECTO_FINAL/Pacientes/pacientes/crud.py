def insertar(nombre, apellido, tel, fecha, correo, tsangre, procedimiento, conexionBD):
    ...
    try:
        ...
        cursor.execute(query, (nombre, apellido, tel, fecha,
                       correo, tsangre, procedimiento))
        conexionBD.commit()
        return True
...
def eliminar(id_pacie, conexionBD):
    if not conexionBD:
        return False
    ...
    try:
        cursor = conexionBD.cursor()
        cursor.execute("DELETE FROM paciente WHERE id = %s", (id_pacie,))
        conexionBD.commit()
        return True
    except Exception as e:
...
def actualizar(nuevo_nombre, nueva_ape, nueva_tel, nuevo_fecha, nuevo_correo, nuevo_tsangre, nuevo_proce, id_pacie, conexionBD):
    if not conexionBD:
        return False
    ...
    try:
        ...
        query = """
            ...
            WHERE id = %s
        """
        cursor.execute(query, (nuevo_nombre, nueva_ape, nueva_tel,
                       nuevo_fecha, nuevo_correo, nuevo_tsangre, nuevo_proce, id_pacie))
        conexionBD.commit()
        return True
    except Exception as e:
