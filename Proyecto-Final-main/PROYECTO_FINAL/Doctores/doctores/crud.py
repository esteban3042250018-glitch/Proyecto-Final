...
def eliminar(id_doc, conexionBD):
    if not conexionBD:
        return False
    ...
    try:
        cursor = conexionBD.cursor()
        cursor.execute("DELETE FROM doctores WHERE id = %s", (id_doc,))
        conexionBD.commit()
        return True
    except Exception as e:
...
def actualizar(nuevo_nombre, nuevo_apellido, nueva_cargo, nueva_tel,  id_doc, conexionBD):
    if not conexionBD:
        return False
    ...
    try:
        cursor = conexionBD.cursor()
        cursor.execute("UPDATE doctores SET nombre = %s,apellido = %s, cargo = %s, tel = %s WHERE id = %s",
                       (nuevo_nombre, nuevo_apellido, nueva_cargo, nueva_tel, id_doc))
        conexionBD.commit()
        return True
    except Exception as e:
