...
def agregarDoctores(conexionBD):
    ...
    while not re.match(r"^\d{10}$", tel):
        tel = input(
            " Teléfono inválido (solo números, 10 dígitos). Intenta de nuevo: ").strip()
...
def vaciarDoctores(conexionBD):
    try:
        ...
        while opc != "si" and opc != "no":
            opc = input(
                "¿Seguro que deseas borrar todos los doctores? (si/no): ").lower().strip()
...
def buscarDoctores(conexionBD):
    ...
    try:
        ...
        if doct and len(doct) > 0:
            _imprimir_tabla_doctores(
                doct, f"🔍 Resultados de Búsqueda: '{nombre}'")
            funciones_doc.espereTecla()
        else:
...
def eliminarDoctores(conexionBD):
    ...
    try:
        ...
        if doct and len(doct) > 0:
            for i in doct:
                # Al incluir 'and not encontrada', nos aseguramos de procesar únicamente
                # la primera coincidencia sin necesitar un break
                if int(i[0]) == id_doc and not encontrada:
                    encontrada = True
                    ...
                    while opc not in ["si", "no"]:
                        opc = input(
                            "¿Deseas borrar al doctor (si/no)? ").lower().strip()

                    if opc == "si":
                        if crud.eliminar(id_doc, conexionBD):
                            funciones_doc.accionExitosa()
                        else:
                            funciones_doc.accionNoExitosa()
                    else:
            ...
            if not encontrada:
                input(
                    "No se encontró ningún doctor con ese ID. Presiona Enter para continuar...")
        else:
            input(
                "...¡No existen doctores registrados!... Presiona Enter para continuar...")
...
def modificarDoctores(conexionBD):
    ...
    try:
        ...
        if doct and len(doct) > 0:
            _imprimir_tabla_doctores(
                doct, "📋 Selecciona un Doctor para Modificar")
            funciones_doc.espereTecla()

            print("\n\t\t...:::: MODIFICAR DOCTORES ::::...\n")
            # 1. Validar que el ID ingresado sea numérico
            ...
            encontrada = False
            for i in doct:
                # 2. Buscar por ID en la primera columna (i[0]) y controlar ejecución sin break
                if int(i[0]) == id_doc and not encontrada:
                    encontrada = True
                    opc = ""
                    while opc not in ["si", "no"]:
                        opc = input(
                            "¿Deseas actualizar el doctor (si/no)? ").lower().strip()
                    ...
                    if opc == "si":
                        ...
                        nueva_cargo = input(
                            f"Nuevo cargo [{i[3]}]: ").upper().strip() or i[3]
                        nueva_tel = input(
                            f"Nuevo teléfono [{i[4]}]: ").upper().strip() or i[4]

                        # 3. Pasar id_doc a la actualización en la BD
                        if crud.actualizar(nuevo_nombre, nuevo_apellido, nueva_cargo, nueva_tel, id_doc, conexionBD):
                            funciones_doc.accionExitosa()
                        else:
                            funciones_doc.accionNoExitosa()
                    else:
            ...
            input(
                "...¡No existen doctores para modificar!... Presiona Enter para continuar...")
