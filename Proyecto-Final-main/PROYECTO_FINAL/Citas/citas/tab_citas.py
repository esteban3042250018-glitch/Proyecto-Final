...
def agregarCitas(conexionBD):
    ...
    while not re.match(r"^[A-ZÁÉÍÓÚÑ ]+$", paciente):
        paciente = (
            input("❌ Nombre inválido. Intenta de nuevo: ").upper().strip())
    ...
    while not fecha_valida:
        try:
            ...
            fecha_input = input(
                "❌ Formato inválido. Usa DD/MM/AAAA HH:MM (Ej. 15/08/2026 14:30): ").strip()
...
def vaciarCitas(conexionBD):
    try:
        ...
        while opc != "si" and opc != "no":
            opc = input(
                "¿Seguro que deseas borrar todas las citas? (si/no): ").lower().strip()
...
def buscarCitas(conexionBD):
    ...
    try:
        ...
        if pacie and len(pacie) > 0:
            _imprimir_tabla_citas(
                pacie, f"🔍 Citas encontradas para: '{nombre}'")
            funciones_citas.espereTecla()
        else:
...
def eliminarCitas(conexionBD):
    ...
    try:
        ...
        if pacie and len(pacie) > 0:
            for i in pacie:
                if int(i[0]) == id_cita and not encontrada:
                    encontrada = True
                    opc = ""
                    while opc != "si" and opc != "no":
                        opc = input(
                            "¿Deseas borrar la cita (si/no)? ").lower().strip()

                    if opc == "si":
                        if crud.eliminar(id_cita, conexionBD):
                            funciones_citas.accionExitosa()
                        else:
                            funciones_citas.accionNoExitosa()
                    else:
...
def modificarCitas(conexionBD):
    ...
    try:
        ...
        if citas and len(citas) > 0:
            _imprimir_tabla_citas(
                citas, "📋 Selecciona una Cita para Modificar")
            funciones_citas.espereTecla()

            print("\n\t\t...:::: MODIFICAR CITA ::::...\n")
            id_str = input("Escribir el ID del doctor a modificar: ").strip()
            while not re.match(r"^\d+$", id_str):
                id_str = input(
                    " ID inválido (solo se permiten números). Intenta de nuevo: ").strip()
            ...
            encontrada = False
            for i in citas:
                if str(i[0]) == id_cita and not encontrada:
                    encontrada = True
                    modificaciones_actuales = i[5]
                    if modificaciones_actuales >= MAX_MODIFICACIONES_CITA:
                        print(
                            f"\n❌ Operación denegada: La cita #{id_cita} ya alcanzó el límite máximo de {MAX_MODIFICACIONES_CITA} modificaciones.")
                        funciones_citas.espereTecla()
                        return
                    opc = ""
                    while opc != "si" and opc != "no":
                        opc = (input(
                            f"¿Deseas actualizar esta cita? (Llevas {modificaciones_actuales}/{MAX_MODIFICACIONES_CITA} cambios) (si/no): ").lower().strip())

                    if opc == "si":
                        print("\nDeje en blanco el campo si no desea cambiarlo:")
                        nuevo_paciente = (
                            input(f"Nuevo paciente [{i[1]}]: ").upper().strip() or i[1])
                        nueva_fecha_input = input(
                            f"Nueva fecha y hora (DD/MM/AAAA HH:MM) [{i[2]}]: ").strip()
                        if nueva_fecha_input:
                            try:
                                obj_fecha = datetime.strptime(
                                    nueva_fecha_input, "%d/%m/%Y %H:%M")
                                nueva_fecha = obj_fecha.strftime(
                                    "%Y-%m-%d %H:%M:%S")
                            except ValueError:
                                print(
                                    "❌ Formato de fecha inválido. Se conservará la fecha anterior.")
                                nueva_fecha = i[2]
                        else:
                            nueva_fecha = i[2]

                        nuevo_doctor = (
                            input(f"Nuevo doctor [{i[3]}]: ").upper().strip() or i[3])
                        nuevo_proce = (
                            input(f"Nuevo procedimiento [{i[4]}]: ").upper().strip() or i[4])

                        if crud.actualizarCita(nuevo_paciente, nueva_fecha, nuevo_doctor, nuevo_proce, id_cita, conexionBD):
                            funciones_citas.accionExitosa()
                        else:
                            funciones_citas.accionNoExitosa()
                    else:
